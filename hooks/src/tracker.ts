...
function monitorContext(
	...
	try {
		...
		if (pct >= 0.8) {
			const extra =
				activeMode !== "ultra"
					? " Switch /oafish ultra to extend session."
					: "";
	...
	try {
		...
		if (
			...
			process.stdout.write(
				JSON.stringify({ decision: "block", reason: statsText }),
			);
			process.exit(0);
		}
		...
		if (activeMode && activeMode !== "off") {
			const contextWarn = monitorContext(
				data.transcript_path as string | undefined,
				activeMode,
			);
			const reinforcement =
