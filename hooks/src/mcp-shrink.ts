...
const SHRINK_FIELDS = new Set(FIELDS_ENV.split(",").map((s) => s.trim()).filter(Boolean));
const MAX_DESC = 120;
...
function withProtected(text: string, fn: (s: string) => string): string {
  ...
  // Restore sentinels in order
  let si = 0;
  result = result.replace(/\x00P(\d+)\x00/g, () => sentinels[si++] ?? "");
  return result;
}
