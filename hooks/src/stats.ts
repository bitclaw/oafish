  ...
  for (const dir of searchDirs) {
    try {
      ...
      for (const name of entries) {
        ...
        const compressedPath = path.join(dir, name.replace(".original.md", ".md"));
        try {
...
export function getStats(opts: StatsOptions): string {
  ...
  if (share) {
    if (!savings) return `oafish: ${metrics.outputTokens.toLocaleString()} output tokens — ${metrics.turns} turns`;
...
function getLifetimeStats(mode: string): string {
