...
const COMPRESSIBLE_EXTS = new Set([".md", ".txt", ".markdown", ".rst", ".typ", ".typst"]);
const SKIP_EXTS = new Set([
  ".py", ".js", ".ts", ".jsx", ".tsx", ".mjs", ".cjs",
  ".json", ".yaml", ".yml", ".toml", ".env", ".lock",
  ".css", ".scss", ".html", ".xml", ".sql",
  ".sh", ".bash", ".zsh", ".fish",
  ".go", ".rs", ".java", ".c", ".cpp", ".h", ".rb", ".php", ".swift",
]);

const SENSITIVE_BASENAME = [
  /^\.env/i, /^credentials/i, /^secrets/i, /^passwords?/i,
...
const SENSITIVE_TOKENS = /secret|credential|password|passwd|apikey|accesskey|token|privatekey/i;
...
async function compressFile(filePath: string): Promise<void> {
  ...
  if (fs.existsSync(backupPath)) {
    process.stderr.write(`oafish: backup already exists: ${backupPath} — delete it first\n`);
    process.exit(1);
  }
  ...
  while (!result.valid && attempt < MAX_RETRIES) {
    ...
    const fix = await client.messages.create({
      model: "claude-haiku-4-5-20251001",
      max_tokens: 4096,
      messages: [{ role: "user", content: buildFixPrompt(original, compressed, result.errors) }],
    });
    compressed = fix.content[0]?.type === "text" ? fix.content[0].text.trim() : compressed;
