...
function digestCode(text: string, filename: string): string {
  ...
  for (const line of lines) {
    ...
    if (
      /^(export\s+)?(async\s+)?function\s+\w|^(export\s+)?(abstract\s+)?class\s+\w|^\s*(public|private|protected|static|async)?\s*(async\s+)?\w+\s*\(/.test(t) &&
      fns.length < 8
    ) {
...
// Digest bash output
function digestBash(response: unknown, cmd: string): string {
  const r = response as Record<string, unknown>;
